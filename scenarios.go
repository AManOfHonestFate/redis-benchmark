...
func SetupContiniousSlotN(rdb *redis.ClusterClient, keys int, n int) {
	ctx, cansel := context.WithTimeout(context.Background(), time.Second)
	module := keys / n
	for i := range keys {
		_, err := rdb.Set(ctx, strconv.Itoa(i%module), i, 0).Result()
		if err != nil {
			cansel()
			log.Fatal("initial set failed", err)
		}
...
func (s *RandomSetsScenario) Setup(rdb *redis.ClusterClient, slots int) {
	s.numberOfSlots = slots
	SetupContinious(rdb, s.numberOfSlots)
}
...
func (s *RandomReadsScenario) Setup(rdb *redis.ClusterClient, slots int) {
	s.numberOfSlots = slots
	SetupContinious(rdb, s.numberOfSlots)
}
...
func (s *RandomReadSetScenario) Setup(rdb *redis.ClusterClient, slots int) {
	s.numberOfSlots = slots
	SetupContinious(rdb, s.numberOfSlots)
}
...
func (s *PipelineScenario) Setup(rdb *redis.ClusterClient, slots int) {
	s.numberOfSlots = slots
	SetupContinious(rdb, s.numberOfSlots)
}
...
func (s *TransactionScenario) Setup(rdb *redis.ClusterClient, slots int) {
	s.numberOfSlots = slots
	SetupContiniousSlotN(rdb, s.numberOfSlots, s.KeysPerSlot)
}
